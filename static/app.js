...
/* ── BOOT ──────────────────────────────────────────────────────────────────── */
// Prevent pywebview/WebView2 from intercepting file drag-drop and navigating away
document.addEventListener('dragover', e => e.preventDefault());
document.addEventListener('drop',     e => {
  e.preventDefault();
  // If the upload modal is open, forward files to the uploader
...
async function confirmReset() {
  try {
    ...
    closeResetModal();
    // Reload the page so the UI reflects empty state
    location.reload();
  } catch (err) {
