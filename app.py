...
# ─── PATH SETUP (handles both script and PyInstaller bundle) ──────────────────

if getattr(sys, 'frozen', False):
    # Running as a PyInstaller bundle — store data in OS app-data folder (hidden from user)
    RESOURCE_DIR = sys._MEIPASS
    if sys.platform == 'win32':
        DATA_DIR = os.path.join(os.environ.get('APPDATA', os.path.expanduser('~')), 'FinanceDashboard')
    elif sys.platform == 'darwin':
        DATA_DIR = os.path.join(os.path.expanduser('~'), 'Library', 'Application Support', 'FinanceDashboard')
    else:
        DATA_DIR = os.path.join(os.path.expanduser('~'), '.local', 'share', 'FinanceDashboard')
else:
    # Running as a normal Python script — keep data local for dev convenience
...
if __name__ == '__main__':
    ...
    t.start()
    time.sleep(0.8)  # give Flask a moment to start before opening the window
